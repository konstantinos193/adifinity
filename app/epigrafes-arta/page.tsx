...
export default function EpigrafesArtaPage() {
  const { t } = useTranslations()

  const services = [
    {
      icon: <Store className="w-8 h-8" />,
      title: t('epigrafes_arta_page.services.store.title') as string,
      description: t('epigrafes_arta_page.services.store.description') as string,
    },
    {
      icon: <Lightbulb className="w-8 h-8" />,
      title: t('epigrafes_arta_page.services.led.title') as string,
      description: t('epigrafes_arta_page.services.led.description') as string,
    },
    {
      icon: <Car className="w-8 h-8" />,
      title: t('epigrafes_arta_page.services.vehicle.title') as string,
      description: t('epigrafes_arta_page.services.vehicle.description') as string,
    },
    {
      icon: <MapPin className="w-8 h-8" />,
      title: t('epigrafes_arta_page.services.wayfinding.title') as string,
      description: t('epigrafes_arta_page.services.wayfinding.description') as string,
    },
  ]

  const benefits = (t('epigrafes_arta_page.benefits') as unknown) as Array<{ title: string; description: string }>

  const faqs = (t('epigrafes_arta_page.faq') as unknown) as Array<{ question: string; answer: string }>
    ...
    <main className="min-h-screen py-20 bg-gradient-to-b from-[#07141C] to-[#0A1A24]">
      ...
      <div className="relative">
        ...
        <div className="container mx-auto px-4 pt-28 pb-20 relative z-10">
          ...
          >
            ...
            >
              {t('epigrafes_arta_page.hero.description')}
            </motion.p>
            ...
            >
              <Link href="/contact#contact-form">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  {t('epigrafes_arta_page.hero.cta')}
              ...
              <Link href="/prints">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  {t('epigrafes_arta_page.hero.view_services')}
                </motion.button>
              </Link>
            </motion.div>
          </motion.div>
      ...
      <div className="container mx-auto px-4 py-16">
        <motion.h2
          ...
          transition={{ duration: 0.8 }}
        >
          {t('epigrafes_arta_page.services.title')}
        </motion.h2>
      ...
      <div className="container mx-auto px-4 py-16">
        <motion.h2
          ...
          transition={{ duration: 0.8 }}
        >
          {t('epigrafes_arta_page.benefits_title')}
        </motion.h2>
      ...
      <div className="container mx-auto px-4 py-16">
        <motion.h2
          ...
          transition={{ duration: 0.8 }}
        >
          {t('epigrafes_arta_page.faq_title')}
        </motion.h2>
        ...
        >
          ...
          <div className="relative z-10">
            <h3 className="text-3xl md:text-4xl font-bold mb-6 text-center text-white">
              {t('epigrafes_arta_page.cta.title')}
            </h3>
            <p className="text-lg text-center max-w-3xl mx-auto mb-8 text-gray-300">
              {t('epigrafes_arta_page.cta.description')}
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <Link href="/contact#contact-form">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  {t('epigrafes_arta_page.cta.button')}
                </motion.button>
              </Link>
            </div>
          </div>
